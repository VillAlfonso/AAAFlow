...
class WanEngine:
    ...
    def _workflow(self, prompt: str, negative: str, *, in_name: str, width: int,
                  height: int, length: int, fps: int, seed: int) -> Dict:
        w = config.WAN
        s = int(w["steps"]); b = int(w["boundary"])
        wf = {
            ...
            "msm_h": {"class_type": "ModelSamplingSD3",
                      "inputs": {"model": ["lora_h", 0], "shift": float(w["shift"])}},
            "msm_l": {"class_type": "ModelSamplingSD3",
                      "inputs": {"model": ["lora_l", 0], "shift": float(w["shift"])}},
    ...
    def animate(self, image_path: str, prompt: str, *, seconds: Optional[float] = None,
        ...
        if not config.wan_ready():
            raise RuntimeError("Wan 2.2 weights are missing — run download_wan14.bat.")
        w = config.WAN
        width = int(width or w["width"]); height = int(height or w["height"])
        ...
        infos = comfy_engine.run_workflow(
            wf, want=("images",), client_id=self._cid, timeout=2400,
            progress=progress, prange=(0.15, 0.95), stage="Animating (Wan 2.2 14B)")
